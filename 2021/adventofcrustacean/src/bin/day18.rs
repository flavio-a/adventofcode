...
fn parse_pair(s: &str) -> Tree {
    ...
    if s.chars().next().unwrap().is_digit(10) {
        return Tree {
            cont: Left(char2int(s.chars().next().unwrap())),
        };
    }
...
fn addside(pair: Tree, val: u8, lr: u8) -> Tree {
    if pair.cont.is_left() {
        return Tree {
            cont: Left(pair.cont.unwrap_left() + val),
        };
    }
    let (sp1, sp2) = *(pair.cont.unwrap_right());
    let newpair = if lr == 2 {
        (sp1, addside(sp2, val, lr))
    } else {
        (addside(sp1, val, lr), sp2)
    };
    return Tree {
        cont: Right(Box::new(newpair)),
    };
}
...
fn explode_(pair: Tree, depth: u8) -> (u8, Tree, u8) {
    ...
    let (sp1, sp2) = *(pair.cont.unwrap_right());
    if depth >= 4 {
        // External call with depth == 0
        // Explode pair
        // print("Exploding pair", pair[1], pair[2])
        return (
            sp1.cont.unwrap_left(),
            Tree { cont: Left(0) },
            sp2.cont.unwrap_left(),
        );
    }
    // Explodes left
...
fn split(pair: Tree) -> (Tree, bool) {
    if pair.cont.is_left() {
        ...
        if v > 9 {
            ...
            let sp2 = Tree {
                cont: Left(if v % 2 == 0 { v / 2 } else { v / 2 + 1 }),
            };
            let newpair = Tree {
                cont: Right(Box::new((sp1, sp2))),
            };
...
fn add(pair1: Tree, pair2: Tree) -> Tree {
    reduce(Tree {
        cont: Right(Box::new((pair1, pair2))),
    })
}
...
fn main() {
    ...
    println!(
        "{}",
        magnitude(it.fold(fst, |acc, elem| add(acc, elem.clone())))
    );
