...
fn parse_fold(s: &&str) -> (bool, i64) {
    ...
    (s.chars().nth(0).unwrap() == 'x', adventofcrustacean::str2int(&(s[2..])))
}

fn fold(dots: Vec<(i64, i64)>, foldline: (bool, i64)) -> Vec<(i64, i64)> {
    let mut newdots = vec![];
    for (x, y) in dots.into_iter() {
        let x1 = if foldline.0 { foldline.1 - (foldline.1 - x).abs() } else { x };
        let y1 = if foldline.0 { y } else { foldline.1 - (foldline.1 - y).abs() };
        newdots.push((x1, y1));
    }
    return newdots;
}
...
fn main() {
    ...
    let dots = tmp.next().unwrap().iter().map(|s| vec2pair(s.split(",").map(adventofcrustacean::str2int).collect())).collect::<Vec<(i64, i64)>>();
    let mut folds = tmp.next().unwrap().iter().map(parse_fold);

    // Part 1
    let dots1 = fold(dots, folds.next().unwrap()).into_iter().unique().collect::<Vec<(i64, i64)>>();
