...
fn compare_scanners(
    sc1: &Vec<DistList>,
    sc2: &Vec<DistList>,
) -> Option<(Rotation, Vec<Option<usize>>)> {
    // Fix the rotation for sc2
    for rot in ALL_ROTATIONS {
        let rot_sc2: &Vec<DistList> = &sc2
            .iter()
            .map(|dl| dl.iter().map(|&p| rotate_point(&rot, p)).collect())
            .collect();
        ...
        for (i, dl_1) in sc1.iter().enumerate() {
            let possible_matches: Vec<usize> = rot_sc2
                .iter()
                .enumerate()
...
fn relative_pos(
    ...
    sc2: &Vec<Point>,
    rot: &Rotation,
    matchings: &Vec<Option<usize>>,
) -> Point {
    let (idx1, optidx2) = matchings
        .iter()
        .enumerate()
        .find(|&(_, v)| v.is_some())
        .unwrap();
...
fn main() {
        ...
        for check_idx in 0..assigned.len() {
            if assigned[check_idx].is_some() {
                ...
                if cusu.is_some() {
                    ...
                    let relpos =
                        relative_pos(&scanners[check_idx], &scanners[round_idx], &rot, &matchings);
                    ...
                    beacons_dists[round_idx] = beacons_dists[round_idx]
                        .iter()
                        .map(|dl| dl.into_iter().map(|&p| rotate_point(&rot, p)).collect())
                        .collect();
                    scanners[round_idx] = scanners[round_idx]
                        .iter()
    ...
    println!(
        ...
        assigned
            .iter()
            .cartesian_product(assigned.iter())
